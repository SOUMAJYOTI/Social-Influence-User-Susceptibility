 ...
 * @version 1.0
 */
public class AdditiveInfModel_beta2 {
	...
	public AdditiveInfModel_beta2(String confPath){
	...
	private void initialData(String confPath) throws IOException {
		...
		System.out.println("Start to fetch diffusion edges from file "+Cons.diffDatFile);
		initiator.createDiffNw(Cons.diffDatFile);
		System.out.println("===========Completd getting diffusion network data===========");
		
		System.out.println("Start to fetch cascade date from file "+Cons.trainDatFile);
		initiator.setNodeCascadeInfo(Cons.trainDatFile);
		initiator.weightNbrs();
	...
	private void setBasicConfig(String confPath) {
		...
		if(config!=null && !config.isEmpty()) {
			...
			Cons.diffDatFile = config.get("diff_edge_file");
...
//			Cons.mom = Double.parseDouble(config.get("momentum"));
			...
			Cons.eta = Double.parseDouble(config.get("eta"));
...
//		}
		...
		private Double[] getSumCAll(List<Integer> sdList) {

			Double[] sdCVecTrust = VecTools.doubleVecInit(Cons.ftuLen);
			Double[] sTrust = VecTools.doubleVecInit(Cons.ftuLen);
			Double[] sTrust_new = VecTools.doubleVecInit(Cons.ftuLen);
			Double[] sTrust_old = VecTools.doubleVecInit(Cons.ftuLen);
			Double[] sTrust_sum = VecTools.doubleVecInit(Cons.ftuLen);
			...
			for(int k=0; k<sdList.size();k++)
				for(int l=0;l<k;l++)
					sumNorm += Cons.nbrWeights[sdList.get(l)][sdList.get(k)];
			
			Double sumDNorm = 0.0;
			if(sumNorm!=0)
				sumDNorm = 1.0/sumNorm;
			else
				sumDNorm = 1.0;
			
			for(int i=0; i<sdList.size();i++){
				for(int j=0;j<i;j++){
					sTrust = VecTools.dotMultiply(Cons.nbrWeights[sdList.get(j)][sdList.get(i)]*sumDNorm, Cons.cMatrix[sdList.get(i)]);
				}
				sTrust_new = VecTools.dotMultiply(1-Cons.eta, sTrust);
				sTrust_old = VecTools.dotMultiply(Cons.eta, Cons.cMatrix[sdList.get(i)]);
				
				sTrust_sum = VecTools.vecAdd(sTrust_new, sTrust_old);
				sdCVecTrust = VecTools.vecAdd(sdCVecTrust, sTrust_sum);
//				sdCVecTrust = VecTools.vecAdd(sdCVecTrust, sTrust);
			}
...
//					System.out.println("Infos: " + infos);
					...
					Double[] sumC = getSumCAll(sidList);
					...
					if(probVal>.0001){
						...
						int sumNorm = 0;
						for(int k=len; k>=Math.max(0,len-Cons.paramL); k--)
							for(int l=Math.max(0,len-Cons.paramL);l<k;l++)
								sumNorm += Cons.nbrWeights[sidList.get(l)][sidList.get(k)];
						...
						for(int i=len; i>=Math.max(0,len-Cons.paramL); i--) {
							int sumT = 0;
							for(int j=Math.max(0,len-Cons.paramL);j<i; j++)
								sumT += Cons.nbrWeights[sidList.get(j)][sidList.get(i)];
							Double[] newU = VecTools.dotMultiply(sumT,  VecTools.dotMultiply(Cons.eta, Cons.uMatrix[calcId]));
							dC[sidList.get(i)] = VecTools.vecAdd(dC[sidList.get(i)], VecTools.dotMultiply(infos.getValue()*Cons.beta*(1-probVal)/probVal, VecTools.dotMultiply(Cons.eta, Cons.uMatrix[calcId])));
							dC[sidList.get(i)] = VecTools.vecAdd(dC[sidList.get(i)], VecTools.dotMultiply(infos.getValue()*Cons.beta*(1-probVal)/probVal, VecTools.dotMultiply(sumDNorm, newU)));
						}
					...
					if(sidList.size()>1) {
						...
						for(int i=len; i>=1; i--) {
							...
							for(int j=Math.max(0,len-Cons.paramL);j<i; j++)
								sumT += Cons.nbrWeights[sidList.get(j)][sidList.get(i)];
							Double[] newU = VecTools.dotMultiply(sumT, Cons.uMatrix[calcId]);
							//-calculate delta C in chained conditional probability when msg received node is activated
							for(Integer sid : subList) {
								dC[sid] = VecTools.vecAdd(dC[sid], VecTools.dotMultiply(-infos.getValue()*Cons.beta, VecTools.dotMultiply(Cons.eta, Cons.uMatrix[calcId])));
								dC[sid] = VecTools.vecAdd(dC[sid], VecTools.dotMultiply(-infos.getValue()*Cons.beta, VecTools.dotMultiply(sumDNorm, newU)));
								
							}
							//-end of delta C calculated
							sumC = getSumCAll(subList);
			...
			if(Cons.negNCasInfo.containsKey(calcId)) {
				...
				for(Map.Entry<String, Integer> infos : snInfos.entrySet()) {
					...
					for(int i=len; i>=1; i--) {
						...
						//-end of delta C calculated
						Double[] sumC = getSumCAll(subList);
						dU[calcId] = VecTools.vecAdd(dU[calcId], VecTools.dotMultiply(-infos.getValue()*Cons.beta, sumC));
					}
					//end of delta U calculated
				}
		...
		private void missionOver() {
			...
			AdditiveInfModel_beta2.missionOver();
		}
	...
	public static void main(String[] args) {
		...
		AdditiveInfModel_beta2 nb = new AdditiveInfModel_beta2(args[0]);
		try {
			nb.learningProc();
		} catch (IOException e) {
