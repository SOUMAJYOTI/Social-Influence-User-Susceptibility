...
//	}
	...
	public void createDiffNw(String filePath) throws IOException{
		BufferedReader br = FileUtil.getBufferReader(filePath);
		String line = null;
		Integer src = null;
		Integer tgt = null;
		while((line=br.readLine())!=null) {
			...
			src = Integer.parseInt(edgeNodes[0]);
			tgt = Integer.parseInt(edgeNodes[1]);
			List<Integer> tmpMap_new = new ArrayList<Integer>();
			
			if(Cons.nodeMap.containsKey(tgt)) {
				tmpMap_new = Cons.nodeMap.get(tgt);
			} 
			
			if(tmpMap_new.contains(src)){
				continue;
			}
			tmpMap_new.add(src);
			Cons.nodeMap.put(tgt, tmpMap_new);
			
		}
//		for(Integer s: Cons.nodeMap.keySet()){
//			List<Integer> nodeList = Cons.nodeMap.get(s);
...
//		}
	...
	public void weightNbrs(){
				
		for(Integer tgt: Cons.posNCasInfo.keySet()){
//			System.out.println(src+"\n");
//			System.out.println(Cons.posNCasInfo.get(src));
			Map<String, Integer> msgNodes = Cons.posNCasInfo.get(tgt);
			for(String n: msgNodes.keySet()){
				String[] sendNodes = n.split("-");
				for(int idx=0; idx<sendNodes.length; idx++){
					String potentialNbr = sendNodes[idx];
					if(potentialNbr.contains("*")){
						Integer nbrIdx = Integer.parseInt(potentialNbr.substring(0, potentialNbr.length()-1));
						
						Map<Integer, Integer> tempMap= Cons.nbrWeights.get(nbrIdx);
						if(tempMap != null){
							Integer tempInt = tempMap.get(tgt);
							Integer newInt;
							if(tempInt != null)
								newInt = tempInt + msgNodes.get(n);
							else
								newInt = msgNodes.get(n);
							tempMap.put(tgt, newInt);
						}
						else{
							tempMap = new HashMap<Integer, Integer>();
							tempMap.put(tgt, msgNodes.get(n));
						}
						Cons.nbrWeights.put(nbrIdx, tempMap);
